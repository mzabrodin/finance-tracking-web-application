            ...
            {selectedReportType === 'summary' && (
              <div className="summary-report-preview">
                <table className="report-table">
                  <tbody>
                    <tr><td>Загальна кількість транзакцій</td><td>{reportData.totalTransactions}</td></tr>
                    <tr><td>Загальні доходи</td><td className="income">{formatAmount(reportData.totalIncome)}</td></tr>
                    <tr><td>Загальні витрати</td><td className="expense">{formatAmount(reportData.totalExpenses)}</td></tr>
                    <tr><td>Чистий баланс</td><td className={reportData.netBalance >= 0 ? 'income' : 'expense'}>{formatAmount(reportData.netBalance)}</td></tr>
                    <tr><td>Кількість доходів</td><td>{reportData.incomeTransactions}</td></tr>
                    <tr><td>Кількість витрат</td><td>{reportData.expenseTransactions}</td></tr>
                    <tr><td>Середня транзакція</td><td>{formatAmount(reportData.averageTransaction)}</td></tr>
                    <tr><td>Найбільший дохід</td><td className="income">{formatAmount(reportData.largestIncome)}</td></tr>
                    <tr><td>Найбільша витрата</td><td className="expense">{formatAmount(reportData.largestExpense)}</td></tr>
                  </tbody>
                </table>
              </div>
            )}
              ...
              <div className="category-budget-report-preview">
                <table className="report-table">
                  ...
                  <tbody>
                    ...
                    {reportData.length === 0 && (
                      <tr><td colSpan="5" className="statistic-empty">Немає даних для відображення</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="detailed-report-preview">
                <table className="report-table">
                  ...
                  <tbody>
                    ...
                    {reportData.length === 0 && (
                      <tr><td colSpan="6" className="statistic-empty">Немає транзакцій для відображення</td></tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="reports-form">
                ...
                <div className="reports-description">
                  <div className="report-type-info">
                    {selectedReportType === 'summary' && (
                      <p>📈 Загальна статистика за обраний період: доходи, витрати, баланс, середні значення</p>
                    )}
                    ...
                    <p><strong>Період:</strong> {reportTransactions.length} транзакцій з {reportDateFrom} по {reportDateTo}</p>
                  </div>
                </div>
              ...
              {monthlyData.length > 0 ? (
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={monthlyData}>
                    ...
                    />
                    <Line type="monotone" dataKey="income" stroke="#00C49F" strokeWidth={3} name="Доходи" />
                    <Line type="monotone" dataKey="expenses" stroke="#FF8042" strokeWidth={3} name="Витрати" />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
